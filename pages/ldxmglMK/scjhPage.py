...
class ScjhPage(Page):
    ...
    def add_jh(self):
        ...
        re_xmmc_value = read_excel_by_cellname(filename=self.filename,
                               sheet_name='劳动项目',
                               cell_num='A2')
        xmmc_eles = self.driver.find_elements_by_xpath(self.xmmc_select)
        for xmmc_ele in xmmc_eles:
            if re.findall('%s.*', xmmc_ele.text, re.S) %re_xmmc_value:
                self.select_box(self.xmmc_select, xmmc_ele.text)
                print '选择   项目名称：%s' %xmmc_ele.text
